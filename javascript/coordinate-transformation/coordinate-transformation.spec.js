...
describe('memoizeTransform', () => {
  ...
  test('should support various argument list', () => {
    const mockFunction = jest.fn((x, y, z) => [x * 2, y * 2, z * 2]);
    const memoizedTransform = memoizeTransform(mockFunction);
    expect(memoizedTransform(1, 1, 1)).toEqual([2, 2, 2]);
    expect(memoizedTransform(2, 2, 2)).toEqual([4, 4, 4]);
    expect(memoizedTransform(1, 1, 1)).toEqual([2, 2, 2]);
    expect(mockFunction).toBeCalledTimes(3);
  });
});
